    ...
    stages {
            ...
            steps {
                script{
                    withEnv(['JENKINS_NODE_COOKIE=dontkill']) {
                        sh "python ${env.startServerScript} --package-url ${env.mcuPackageUrl} --package-name mcu-all-bin-v{data}.tgz  --base-dir ${env.mcuServerBasePath} --git-branch ${GIT_BRANCH} --owner zhangyihui1 --repo test-jenkins --commit-id ${GIT_COMMIT} --github-script /home/webrtctest3/zyh_github_test/github_checks/push_result.py --mode mcu"
                    }
                }
            }
        }
        stage('startP2PServer'){
            ...
            steps{
                script{
                    withEnv(['JENKINS_NODE_COOKIE=dontkill']) {
                        sh "python ${env.startServerScript} --p2p-server-path ${env.p2pServerPath} --owner zhangyihui1 --repo test-jenkins --commit-id ${GIT_COMMIT} --github-script /home/webrtctest3/zyh_github_test/github_checks/push_result.py --mode p2p"
                        }    
                }
            }
        }
